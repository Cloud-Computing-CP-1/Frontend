...
import { FiArrowUpRight, FiBox, FiExternalLink, FiGitBranch, FiPlay, FiRefreshCw, FiSettings, FiTerminal } from "react-icons/fi";
...
export default function ProjectActivity({ project, expanded = false }: { project: Project; expanded?: boolean }) {
  ...
  const liveUrl = /^https?:\/\//.test(project.deployment_url || "") ? project.deployment_url : null;
  ...
  return <section className="dfw-panel dfw-activity">
    ...
    <div className="dfw-activity-body">
      ...
      {liveUrl && <a href={liveUrl} target="_blank" rel="noreferrer" className="dfw-text-link">Visit application <FiExternalLink /></a>}
      ...
      <div className="dfw-subheading"><h3>Latest deployment</h3></div><p className="dfw-muted dfw-small">Deployment history is not available yet. Current project status and saved builds are shown here.</p>
